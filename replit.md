...
- **AI-Powered Assistance**:
    ...
    - **AI Task & Note Automation (Smart Gemini)**: Generates descriptive notes, automatically creates tasks from email threads with confidence-based filtering, intelligent deduplication, status auto-update, knowledge base integration, rate limiting & retry, multi-level caching, real-time and periodic processing, respects manual modifications.
    ...
    - **AI-Powered Financial Transaction Detection**: 3-level resilient detection system (Gemini AI, OCR Fallback, Manual Review Queue) with circuit breaker pattern. Automated detection of payments and expenses from email attachments, creating financial suggestions requiring user approval. Includes intelligent duplicate detection and configurable UI toggles.
